...
1. **Welcome** (`/`) — First-run: brief What is Atelier + Create profile only (no freemium/Premium pitch, no Add to closet, **no home tour**). Returning signed-in redirects to **`/lookbook`**. After budget: first storeFirst Generate starts in the background while a one-time spotlight tour (Style → Lookbook → Closet → Profile, `atelier.postProfileDemo.v1`) runs; finish lands **`/style`**. First board auto storeFirst only once (`!hasUsedFreeBoard` / no prior request / no look votes); later Generates need an explicit CTA.
...
3. **Request looks** — profile is saved as `atelier.stylistRequest.v1`. Style Site’s bot talks to Stylist; it **POSTs** `atelier.stylistResponse.v1` to `/api/stylist/looks`. Lookbook shows a pending state until that ingest, then a magazine shop-the-look **board** (fitting | product collage; prices and shop stay below).
4. **Freemium** — first unpaid board only (`hasUsedFreeBoard`). Still **4 looks**. Looks 1–3 include fittings; look 4 is shoppable with **See it on** (Premium sheet). Request sends `freeFirstBoard: true` and `tier: "free"` so Stylist gens 3 fittings. Regenerates / later boards are Premium (mock unlock, no Stripe).
5. **Likes** — Wear / Maybe / No (or like / skip / dislike) write that user’s lookbook on **You**. Wear and Maybe leave the active board into Saved; No leaves too. Active board keeps unvoted looks only. Shared hero-cache (`fingerprint × pieceIds`) is unchanged.
6. **Closet** — Manual or Photo (cutout). Free cap **10** pieces. The 11th opens a mock upgrade that ends on **Development in progress** — no Stripe. Video and Elevate are parked. Capsule expand is mock-only. After the first lookbook, a skippable invite points here once.
7. **Tabs (Friend beta four-bucket)** — Style (`/style`, generate + active board) · Lookbook (`/lookbook`, Wear+Maybe saved archive) · Closet (`/closet`) · Profile (`/profile`; `/you` redirects). Optional shape later from Profile. When Generate runs and the closet is non-empty, `atelier.stylistRequest.v1` includes `closet[]` (ids, cutout images, roles). Stylist mixes closet + store (see [docs/stylist-contract.md](docs/stylist-contract.md)).
