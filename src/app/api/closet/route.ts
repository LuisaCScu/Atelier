...
export async function POST(req:NextRequest){
 ...
 try{
  ...
  if(body.action!=='cutout'||!body.item||typeof body.item.role!=='string'||!roles.includes(body.item.role)||typeof body.item.name!=='string'||body.item.name.length>300)return NextResponse.json({error:'Choose a garment first.'},{status:400});
