...
export async function POST(req:NextRequest){
 ...
 const {image}=JSON.parse(raw);if(typeof image!=='string'||!/^data:image\/(jpeg|png|webp);base64,[A-Za-z0-9+/=]+$/.test(image))return NextResponse.json({error:'Choose a clear face photo.'},{status:400});
